...
@app.get("/", tags=["READ"],response_model=list[ResponseModel])
def root():
...
@app.get("/students/name/{name}", tags=["READ"],response_model=list[ResponseModel])
...
@app.get("/students/id/{id}", tags=["READ"],response_model=list[ResponseModel])
...
def get_students_by_age(min_age:int):
    ...
    students_serialized=[student_helper(s) for s in students]
    return {"students":students_serialized}
    # print("RES: ", response)
    # return response
...
def get_students_by_name_starts(letter:str):
    ...
    serialized_students=[student_helper(s) for s in students]
...
@app.post("/students/batch", tags=["CREATE"],response_model=list[ResponseModel])
...
# UPDATE
@app.put("/students/update/{name}", tags=["UPDATE"],response_model=list[ResponseModel])
...
# DELETE
@app.delete("/students/student_name/{name}", tags=["DELETE"],response_model=list[ResponseModel])
...
@app.delete("/students/student_id/{id}", tags=["DELETE"],response_model=list[ResponseModel])
